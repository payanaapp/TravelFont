...
# install venv and activate virtualenv
RUN apt-get update && apt-get install -y python3-venv && python3 -m venv .venv && source .venv/bin/activate
...
RUN pip3 install --break-system-packages -r $travelfont_home/payana/payana_bl/requirements.txt
RUN pip3 install --break-system-packages -r $travelfont_home/payana/payana_service/requirements.txt
RUN pip3 install --break-system-packages -r $travelfont_home/payana/payana_core/requirements.txt
# install the below package separately as it fails if installed from requirements.xt
RUN pip3 install --break-system-packages --upgrade google-api-python-client google-auth-httplib2 google-auth-oauthlib
