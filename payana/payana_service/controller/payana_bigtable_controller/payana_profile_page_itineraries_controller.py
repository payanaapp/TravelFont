...
profile_page_itinerary_name_space = Namespace(
    'itinerary', description='Manage profile page itineraries')
...
@profile_page_itinerary_name_space.route("/")
class PayanaProfileTableEndPoint(Resource):

    @profile_page_itinerary_name_space.doc(responses={200: payana_200_response, 400: payana_400_response, 500: payana_500_response})
    ...
    def get(self):
        ...
        payana_profile_read_obj = PayanaBigTable(payana_profile_page_itinerary_table)
        ...
        if len(payana_profile_read_obj_dict) == 0:
            raise Exception(payana_empty_row_read_exception,
                            profile_page_itinerary_name_space)
    ...
    def post(self):
        ...
        profile_page_itineraries_object = request.json

        payana_profile_page_itineraries_object = PayanaProfilePageItineraryTable(**profile_page_itineraries_object)
        payana_profile_page_itineraries_obj_write_status = payana_profile_page_itineraries_object.update_payana_profile_page_itinerary_bigtable()

        if not payana_profile_page_itineraries_obj_write_status:
            raise Exception(
                payana_profile_table_create_failure_message_post, profile_page_itinerary_name_space)
    ...
    def put(self):
        ...
        if not payana_profile_obj_edit_status:
            raise Exception(
                payana_profile_table_write_failure_message_post, profile_page_itinerary_name_space)
